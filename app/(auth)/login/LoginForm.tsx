...
function LoginContent({ org }: { org: OrgBranding | null }) {
  ...
  const bgColor = org?.color_primario ?? '#1B2A4A'
  const accentColor = org?.color_acento ?? '#C9A84C'
  const logoSrc = org?.logo_url ?? '/logo.png'
  const logoAlt = org?.nombre ?? 'Ministerio de Economía'
  const subtitulo = org?.texto_sidebar ?? 'Sistema de Seguimiento de Proyectos'
  const footer = org?.nombre ?? 'Ministerio de Economía — República Argentina'
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative" style={{ backgroundColor: bgColor }}>
      <div className="w-full max-w-md">
        ...
        <div className="flex flex-col items-center mb-10">
          ...
          <p className="text-sm mt-1 tracking-widest uppercase" style={{ color: accentColor }}>
            {subtitulo}
          </p>
        </div>
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4 mb-6">
            <div>
              ...
              <input
                ...
                required
                autoComplete="email"
                className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-white/30 focus:outline-none transition-colors text-sm"
              />
            </div>
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 pr-12 text-white placeholder-white/30 focus:outline-none transition-colors text-sm"
                />
            ...
            <button type="submit" disabled={loading}
              className="w-full disabled:opacity-50 font-semibold py-3 px-4 rounded-xl transition-all duration-200 text-sm mt-2"
...
export default function LoginForm({ org }: { org: OrgBranding | null }) {
  return (
    <Suspense>
      <LoginContent org={org} />
    </Suspense>
  )
}
