          ...
          {proyectosFiltrados.map(p => {
            ...
            return (
              <div key={p.id} className="bg-white rounded-xl border border-gray-100 p-6">
                ...
                {/* Líneas ordenadas */}
                {lineas.length === 0 ? (
                  <p className="text-gray-300 text-sm italic mt-4">Sin líneas de acción</p>
                ) : (
                        ...
                        <div key={l.id} className={`rounded-xl border-2 p-4 ${ESTADO_BORDER[er] ?? 'border-gray-200 bg-gray-50'}`}>
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-bold text-[#1B2A4A] uppercase tracking-wider">
                              Línea {['I','II','III'][l.orden - 1] ?? l.orden}
                            </span>
                            <StatusBadge estado={er} size="sm" />
                          </div>
                          <h3 className="font-semibold text-[#1B2A4A] text-sm mb-1">{l.nombre}</h3>
                          {l.descripcion && (
                            <p className="text-xs text-gray-500 mb-2">{l.descripcion}</p>
                          )}
                          {l.metrica_exito && (
                            <p className="text-xs text-gray-400 italic mb-2">✓ {l.metrica_exito}</p>
                          )}
                ...
                {lineas.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    <div className="flex items-center justify-between text-xs text-gray-400 mb-1.5">
                      <span>{lineas.filter((l: any) => calcularEstadoReal(l.estado, l.fecha_fin) === 'completado').length}/{lineas.length} líneas completadas</span>
                      <span>{Math.round((lineas.filter((l: any) => calcularEstadoReal(l.estado, l.fecha_fin) === 'completado').length / lineas.length) * 100)}%</span>
                    </div>
                    <div className="bg-gray-100 rounded-full h-1.5">
                      <div className="bg-[#1B2A4A] h-1.5 rounded-full transition-all"
                        style={{ width: `${(lineas.filter((l: any) => calcularEstadoReal(l.estado, l.fecha_fin) === 'completado').length / lineas.length) * 100}%` }} />
                    </div>
                  </div>
                )}
              </div>
