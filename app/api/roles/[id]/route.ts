...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  if (nombre || descripcion) {
    await supabaseAdmin.from('roles').update({ nombre, descripcion }).eq('id', params.id)
  }
  if (permisos) {
  ...
  const { data } = await supabaseAdmin.from('roles').select('*, permisos:permisos_rol(*)').eq('id', params.id).single()
